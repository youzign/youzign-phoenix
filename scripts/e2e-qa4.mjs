// Visual end-to-end QA RE-RUN (qa4) for the legacy design import flow.
//
...
const log = (...a) => console.log("[e2e-qa4]", ...a);
...
const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "yz-qa4-chrome-"));
...
try {
  for (const identifier of ACCOUNTS) {
    ...
    fs.writeFileSync(shot("qa4-results.json"), JSON.stringify({ ...allResults, runErrors }, null, 2));
  }
  ...
  fs.writeFileSync(shot("qa4-results.json"), JSON.stringify(allResults, null, 2));
  await b.close();
  try { fs.rmSync(userDataDir, { recursive: true, force: true }); } catch {}
  console.log("RESULTS WRITTEN:", shot("qa4-results.json"));
  console.log("QA4 SCRIPT DONE");
} catch (err) {
  await b.close();
  try { fs.rmSync(userDataDir, { recursive: true, force: true }); } catch {}
  console.error("QA4 SCRIPT FAILED", err);
  fs.writeFileSync(shot("qa4-results.json"), JSON.stringify({ ...allResults, runErrors, fatalError: String(err?.stack || err) }, null, 2));
  process.exit(1);
}
