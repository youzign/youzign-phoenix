...
// Switch to the crop-curve fixture.
await page.select("select", "crop-curve.xml");
await page.evaluate(async () => { await document.fonts.ready; });
await sleep(1500);

// Select the curved text (apex near upper-centre) so the Curve control shows.
const canvas = await page.$(".yz-canvas");
const box = await canvas.boundingBox();
await page.mouse.click(box.x + box.width * 0.5, box.y + box.height * 0.3);
await sleep(600);

await page.screenshot({ path: resolve(shotsDir, "editor-v0.2-curvedtext.png") });
console.log("saved editor-v0.2-curvedtext.png");
