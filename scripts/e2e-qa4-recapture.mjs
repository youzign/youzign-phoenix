// Recapture helper for e2e-qa4: re-imports a subset of designs per account and
// screenshots the editor canvas with a LARGE viewport so tall/large canvases
// are captured without the editor UI overlapping the element screenshot.
// Fresh throwaway Chrome profile so nothing from a prior run lingers.
// Run: node scripts/e2e-qa4-recapture.mjs
...
const SHOT_DIR = "docs/planning/legacy-claim-shots/qa4";
const TARGETS = {
  imgllc: ["Kindle cover e", "VirtualCombinationLockContactUS", "Inform", "ISSPWhichTypeofAccount", "ISSP Tranparent"],
  mackcarbon: ["Sales 3", "15x100 mtb vork 2021 grijze achtergrond", "BOTTLE CAGES FRONT PIC", "Weight Weenies flatbar opening", "Weight weenies riserbar opening pic", "Thor Hammer topcap", "zadel handvast aangedraaid"],
  "7YgkgPFJ": ["Boxroom Pineapple", "Boxroom Business Card", "OMS flyer back", "Unl eng v spain", "snatchems beerfest pod", "HiQ beerfest voucher", "SandJ security services wide"],
};

const log = (...a) => console.log("[qa4-recapture]", ...a);
const shot = (n) => path.join(SHOT_DIR, n);
const slug = (s) => (s || "untitled").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 40) || "untitled";

const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "yz-qa4rec-chrome-"));
...
try {
  ...
  console.log("QA4 RECAPTURE DONE");
} catch (e) {
  await b.close();
  try { fs.rmSync(userDataDir, { recursive: true, force: true }); } catch {}
  console.error("QA4 RECAPTURE FAILED", e);
  process.exit(1);
}
